var popWin = {
    scrolling: 'no',
    //是否显示滚动条 no,yes,auto
    int: function () {
        this.mouseClose();
        this.closeMask();
    },

    showWin: function (width, height, title,src) {

        var marginLeft = width / 2;
        var marginTop = height / 2;
        var inntHtml = '';
        inntHtml += '<div id="mask" style="width:100%; height:100%; position:fixed; top:0; left:0; z-index:1999;background:#cccccc; filter:alpha(opacity=50); -moz-opacity:0.5; -khtml-opacity: 0.5; opacity:0.5;"></div>';
        inntHtml += '<div id="maskTop" style="width: ' + width + 'px; height: ' + height + 'px; border: #999999 1px solid; background: #fff; color: #333; position: fixed; top: 50%; left: 50%; margin-left: -' + marginLeft + 'px; margin-top: -' + marginTop + 'px; z-index: 2999; filter: progid:DXImageTransform.Microsoft.Shadow(color=#909090,direction=120,strength=4); -moz-box-shadow: 2px 2px 10px #909090; -webkit-box-shadow: 2px 2px 10px #909090; box-shadow: 2px 2px 10px #909090;">';
        inntHtml += '<div id="maskTitle" style="height: 40px; line-height: 40px; font-family: Microsoft Yahei; font-size: 20px; color: #F8F8F8; padding-left: 20px; background:#474a5a">';
        inntHtml += '' + title + '';
        inntHtml += '<div id="popWinClose" style="width: 28px; height: 28px; cursor: pointer; position: absolute; top: -12px; right: -9px; background-image: url(data:image/png;base64,);"></div>';

        inntHtml += '<div><label id="mind" style="width=280px;height=60px;padding-left: 20%;font-family: Microsoft Yahei;font-size:18px;color:#474a5a">您确定要删除下列图片吗？</label></div>';

        inntHtml += '<div class="deleteImgList" style="float:left;height:120px;width:95%;text-align:center;white-space: nowrap;overflow-x: auto;overflow-y:hidden">';
        for(var i=0;i<src.length;i++) {
            inntHtml += '  <img src="' + src[i] + '" style="width:80px;height:80px;cursor: pointer;padding:20px 10px 15px;" >';
        }
        inntHtml += '</div>';

        inntHtml += '<div class="opration" style="margin-top:120px;white-space:nowrap">' ;
        inntHtml += '<label class="sureBtn" style="width:60px;height:20px;cursor: pointer;margin-left: 33%;color:#474a5a">确定</label>';
        inntHtml += '<label class="cancelBtn" style="cursor: pointer;margin-left: 10%;color:#474a5a">取消</label>';
        inntHtml += '</div>';
        inntHtml += '</div>';

        $("body").append(inntHtml);
        this.int();
    },

    mouseClose: function () {
        $("#popWinClose").on('mouseenter',
            function () {
                $(this).css("background-image", "url(data:image/png;base64,)");

            });

        $("#popWinClose").on('mouseleave',
            function () {
                $(this).css("background-image", "url(data:image/png;base64,)");
            });
    },


    closeMask: function () {

        $("#popWinClose").on('click',
            function () {
                $("#mask,#maskTop").fadeOut(function () {
                    $(this).remove();
                });
            });
        $(".sureBtn").on('click',
            function(){
                $("#mask,#maskTop").fadeOut(function () {
                    $(this).remove();
                });
                deleteAffixFromServer();
            });

        $(".cancelBtn").on('click',
            function () {
                $("#mask,#maskTop").fadeOut(function () {
                    $(this).remove();
                });
            });

    }
}