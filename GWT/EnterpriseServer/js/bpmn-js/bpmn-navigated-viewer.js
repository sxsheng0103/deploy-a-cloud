...
function BpmnRenderer(eventBus, styles, pathMap, priority) {
  ...
  function renderExternalLabel(p, element, align) {
    ...
    return renderLabel(p, semantic.name, { box: element, align: align, style: { fontSize: '11px' }, isSeq:isSeq });
  }
...
Canvas.prototype._centerViewport = function(center,width,height,firstRun) {
  ...
  // the diagram origin instead
  	...
  	if (firstRun) {	 
		...
		if(typeof(root)!= "undefined"){
	        var elements = this.getRootElement().children;
            var obj = this.getElenmentsBounds(elements);
		    if(typeof(obj.x) != "undefined" && !isNaN(obj.x)) inner.x = obj.x;
		    if(typeof(obj.y) != "undefined" && !isNaN(obj.y)) inner.y = obj.y;
		    if(typeof(obj.width) != "undefined" && !isNaN(obj.width)) inner.width = obj.width;
		    if(typeof(obj.height) != "undefined" && !isNaN(obj.height)) inner.height = obj.height;
		}
	}
...
//获取内部元素的宽和高
Canvas.prototype._getInnerScale = function(firstRun) {
...
//centerDisplay 将图形居中并以原始尺寸显示 by zjq
Canvas.prototype.centerDisplay = function (width,height){
    this._changeViewbox(function(width,height) {
        var elements = this.getRootElement().children;
        var obj = this.getElenmentsBounds(elements);
        var h = (height - obj.height)/2;
        var w = (width - obj.width)/2;
        setCTM(this._viewport.node,{a:"1",b:"0",c:"0",d:"1",e: -obj.x + w, f:-obj.y + h });
    });
};

//获取元素集合的位置和大小 by zjq
Canvas.prototype.getElenmentsBounds = function (elements){
        var minX,
            maxX,
            minY,
            maxY;
        ...
        while(elements[i]){
            ...
            if(type == "bpmn:SequenceFlow"){
                //线
                var j = 0;
                while(element.waypoints[j]){
                    if(minX == null || element.waypoints[j].x < minX ){
                        minX =  element.waypoints[j].x;
                    }
                    if(maxX == null || element.waypoints[j].x > maxX){
                        maxX = element.waypoints[j].x;
                    }
                    if(minY == null || element.waypoints[j].y < minY){
                        minY  = element.waypoints[j].y;
                    }
                    if(maxY == null || element.waypoints[j] > maxY){
                        maxY = element.waypoints[j].y;
                    }
                    j++;
                }
            }else{
                //图形
                if(minX == null || element.x < minX ){
                    minX =  element.x;
                }
                if(maxX == null || element.x + element.width> maxX){
                    maxX = element.x + element.width;
                }
                if(minY == null || element.y < minY){
                    minY  = element.y;
                }
                if(maxY == null || element.y + element.height > maxY){
...
Text.prototype.createText = function(parent, text, options) {
  ...
  forEach(layouted, function(line) {
	...
	if(isSeq){
		 var tspan = Snap.create('tspan', { x: x, y: y-10 }).node;
	}else{
